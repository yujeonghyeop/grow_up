const quotes =[ //적고싶은 글귀들을 배열로 저장한다.
{
...
const author = document.querySelector("#quote span:last-child") //html에 있는 곳에 적어두기 위해 queryselector로 가져온 다음 변수로 지정해준다. span last child
const todayquotes = quotes[Math.floor(Math.random()*(quotes.length))]  //math.floor(버림)을 사용해서 random*quotes.length로 생성된 랜덤숫자를 버려준다.

quote.innerText = todayquotes.quote //quotes배열의 quote를 innertext에 적음
